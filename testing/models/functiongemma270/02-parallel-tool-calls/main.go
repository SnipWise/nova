...
func main() {
	...
	agent, err := tools.NewAgent(
		...
		agents.Config{
			...
			SystemInstructions: `You are a function-calling AI assistant.
			Your primary duty is to detect when tool/function calls are needed based on user requests 
			and invoke the appropriate tools with correct parameters.
			When a user asks you to perform tasks like calculations or greetings, 
			you must identify the corresponding tool and call it with the proper arguments. 
			Always respond using available tools rather than generating text responses for tasks that tools can handle.
			`,
		},
		...
		models.Config{
			...
			//Name: "huggingface.co/unsloth/functiongemma-270m-it-gguf:bf16",
