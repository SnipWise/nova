...
When you use `WithOrchestratorAgent`, the gateway **automatically configures routing** using the orchestrator's `GetAgentForTopic` method. You don't need to provide `WithMatchAgentIdToTopicFn` unless you have custom routing logic.
...
2. **If no custom `matchAgentIdToTopicFn` is provided**: The gateway automatically calls `orchestratorAgent.GetAgentForTopic(topic)` to get the agent ID.
3. **If a custom `matchAgentIdToTopicFn` is provided**: The gateway uses your custom function instead.
4. The gateway switches to the matched agent if different from the current one.
