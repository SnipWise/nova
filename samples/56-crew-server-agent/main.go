...
func main() {
	...
	crewServerAgent, err := crewserver.NewAgent(
		...
		crewserver.WithCompressorAgentAndContextSize(compressorAgent, 7000),
	...
	if err := generateWebConfig(crewServerAgent.GetPort()); err != nil {
		display.Errorf("⚠️  Failed to generate web config: %v\n", err)
	} else {
		display.Colorf(display.ColorGreen, "✅ Generated web/config.js with API URL\n")
	}
...
// generateWebConfig creates a config.js file with the API URL
func generateWebConfig(apiPort string) error {
...
window.NOVA_API_URL = 'http://localhost%s';
`, apiPort)

	configPath := "./web/config.js"
