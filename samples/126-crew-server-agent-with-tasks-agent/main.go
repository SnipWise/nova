...
func main() {
	...
	display.Colorf(display.ColorCyan, "🚀 Crew Server Agent with Tasks Agent\n")
	...
	fmt.Println("and execute each task step by step via SSE streaming.")
	fmt.Println()
	fmt.Println("Usage with curl:")
	fmt.Println(`  curl -N http://localhost:9090/completion \`)
	fmt.Println(`    -H "Content-Type: application/json" \`)
	fmt.Println(`    -d '{"question":"Create a project directory called demo and read the README.md file"}'`)
