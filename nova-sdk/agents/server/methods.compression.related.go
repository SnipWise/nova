...
func (agent *ServerAgent) CompressChatAgentContextIfOverLimit() (int, error) {
	...
	if agent.chatAgent.GetContextSize() > agent.contextSizeLimit {
		agent.log.Info("Chat agent context size %d exceeds limit of %d; compressing...", agent.chatAgent.GetContextSize(), agent.contextSizeLimit)

		newContext, err := agent.compressorAgent.CompressContext(agent.chatAgent.GetMessages())
		if err != nil {
			return 0, err
		}
