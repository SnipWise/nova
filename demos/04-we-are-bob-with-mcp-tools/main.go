...
func GetCoderAgent(ctx context.Context, engineURL string) (*chat.Agent, error) {
	...
	coderAgent, err := chat.NewAgent(
		...
		models.Config{
			...
			Temperature: models.Float64(coderAgentModelTemperature),
		},
...
func GetThinkerAgent(ctx context.Context, engineURL string) (*chat.Agent, error) {
	...
	thinkerAgent, err := chat.NewAgent(
		...
		models.Config{
			Name:        thinkerModel,
			Temperature: models.Float64(thinkerModelTemperature),
		},
	)
	if err != nil {
		return nil, err
...
func GetTranslatorAgent(ctx context.Context, engineURL string) (*chat.Agent, error) {
	...
	translatorAgent, err := chat.NewAgent(
		...
		models.Config{
			...
			Temperature: models.Float64(translatorModelTemperature),
		},
	)
	if err != nil {
		return nil, err
...
func GetGenericAgent(ctx context.Context, engineURL string) (*chat.Agent, error) {
	...
	genericAgent, err := chat.NewAgent(
		...
		models.Config{
			Name:        genericModel,
			Temperature: models.Float64(genericModelTemperature),
		},
	)
	if err != nil {
		return nil, err
...
func GetRagAgent(ctx context.Context, engineURL string) (*rag.Agent, error) {
	...
	ragModel := env.GetEnvOrDefault("RAG_MODEL", "hai/mxbai-embed-large")
	...
	generatingVectorsSpinner.Success("done!")
...
func main() {
	...
	for {
		...
		resultOfToolCalls, err := toolsAgent.DetectToolCallsLoop(
			[]messages.Message{
				{
					Role:    roles.User,
					Content: question,
